function patchModel = BuildPatchModel( trainImageDataList, params )
    ...
    if genSVMData == true
        ...
        for p = 1:numPatches
           ...
           for i = 1:numTrainData
            ...
            mkdir( '../data/svm-data-imm' );
            fname = sprintf('../data/svm-data-imm/pos%d.mat', p);
            pat = posPatches(1:posCnt, :);
            save(fname, 'pat');

            fname = sprintf('../data/svm-data-imm/neg%d.mat', p);
    ...
    %% Train SVM
    for i=1:numPatches
        fname = sprintf('../data/svm-data-imm/pos%d.mat', i);
        posPatches = load(fname);

        fname = sprintf('../data/svm-data-imm/neg%d.mat', i);
...
%         end
        ...
        if(ShowSVMWeights)
            ...
            wname = sprintf('../data/svm-data-imm/weight_%d.bmp', i-1);
            imwrite(kw, wname);
        end
