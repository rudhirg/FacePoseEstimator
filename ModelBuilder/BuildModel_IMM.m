...
annotatedFileDir = '../data/imm_face_db/';
% folder containing the training images
trainImgFolder = '../data/imm_face_db/';

% get the name of all files in images folder
imageFileList = dir( fullfile(trainImgFolder,'*.jpg') );
% get the name of all files in points folder
pointFileList = dir( fullfile(annotatedFileDir,'*.asf') );
...
for val = imageFileDict.keys()
    ...
    [x, y] = ReadImagePoints_IMM(trainImageData.filePointName, annotatedFileDir);
...
%% build patch model
patchModel = BuildPatchModel_IMM( trainImageDataList, params );
